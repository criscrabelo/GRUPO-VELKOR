...
- Sessão persistida em `localStorage` (padrão do supabase-js, com auto-refresh de token). Implicação: um ataque XSS bem-sucedido poderia ler o token. Mitigações atuais: React escapa todo conteúdo dinâmico, não há renderização de HTML vindo de usuário, o único `dangerouslySetInnerHTML` do projeto injeta um JSON-LD estático, e agora há Content Security Policy (ver abaixo) bloqueando o vetor mais comum de injeção — carregar script de outro domínio.
- **Content Security Policy configurada** (`next.config.mjs`, cabeçalhos aplicados a todas as rotas): `default-src 'self'`, `connect-src` liberado só para `'self'` e `https://*.supabase.co`, `frame-ancestors 'none'` (bloqueia clickjacking), `object-src 'none'`, mais `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy: strict-origin-when-cross-origin`, `Permissions-Policy` restritiva (câmera/microfone/geolocalização/pagamento desligados) e `Strict-Transport-Security` (só tem efeito quando servido via HTTPS). **Decisão registrada**: cheguei a implementar CSP com nonce por requisição + `strict-dynamic` (o padrão mais forte, via middleware), mas testando no navegador ele quebrava a própria hidratação do Next — quase todo o site é pré-renderizado no build (`○ Static`), e um nonce só faz sentido por requisição; em página estática ele é gravado uma vez no HTML do build e nunca muda, então os `<script>` do próprio Next (chunks e o payload de hidratação) ficavam sem nonce válido e eram bloqueados pelo próprio CSP. A política final usa `script-src 'self' 'unsafe-inline'` e `style-src 'self' 'unsafe-inline'` (este último necessário porque vários componentes usam `style={{...}}` para valores dinâmicos, ex. gradientes e largura de barra de progresso — nonce não cobre atributos `style`). Testado sem nenhuma violação de CSP no console, nas 6 rotas principais e nos fluxos de diagnóstico/simulador.
...
- Cabeçalhos de segurança em todas as rotas (`next.config.mjs`): Content-Security-Policy, X-Content-Type-Options, X-Frame-Options, Referrer-Policy, Permissions-Policy e Strict-Transport-Security (seção 6)
